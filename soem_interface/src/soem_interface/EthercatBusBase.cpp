...
namespace soem_interface
{
   ...
   */
    ...
    for (auto &slave : slaves_)
    {
      slave->autoConfigurePdoSizes();
    }

    bool l_hasDc = ecx_configdc(&ecatContext_);
    if (l_hasDc)
      MELO_INFO_STREAM(logger_, "DC enabled!");
  ...
  //------------------------------------------------------
  void EthercatBusBase::ecatCheck()
  {
    if (!workingCounterIsOk())
    {
      ...
      for (int slave = 1; slave <= getNumberOfSlaves(); slave++)
      {
        ecatContext_.grouplist[slave].docheckstate = false;
        if (ecatContext_.slavelist[slave].state != EC_STATE_OPERATIONAL)
        {
          ...
          else if (ecatContext_.slavelist[slave].state > EC_STATE_NONE)
          {
  ...
  void EthercatBusBase::updateRead()
  {
    ...
    if (!workingCounterIsOk())
    {
      ...
      // MELO_DEBUG_THROTTLE_STREAM(logger_, 1.0, "Update Read:" << this->getName());
      // MELO_DEBUG_THROTTLE_STREAM(logger_, 1.0, "Working counter is too low: " << wkc_.load() << " < " << getExpectedWorkingCounter());
      return;
    }
  ...
  void EthercatBusBase::printALStatus(const uint16_t slave)
  {
    ...
    MELO_INFO_STREAM(logger_, " slave: " << slave << " alStatusCode: 0x" << std::setfill('0') << std::setw(8) << std::hex << ecatContext_.slavelist[slave].ALstatuscode << " " << ec_ALstatuscode2string(ecatContext_.slavelist[slave].ALstatuscode));
  }
  ...
  void EthercatBusBase::syncDistributedClock01(const uint16_t slave, const bool activate, const double cycleTime0, const double cycleTime1, const double cycleShift)
  {
    MELO_INFO_STREAM(logger_, "Bus '" << name_ << "', slave " << slave << ":  " << (activate ? "Activating" : "Deactivating")
                                      << " distributed clock synchronization...");

    ecx_dcsync01(&ecatContext_, slave, static_cast<uint8_t>(activate), static_cast<uint32_t>(cycleTime0 * 1e9), static_cast<uint32_t>(cycleTime1 * 1e9),
                 static_cast<int32_t>(1e9 * cycleShift));
